...
class UserFixture(BaseModel):
    ...
    # добавили новое свойство authentication_user
    @property
    def authentication_user(self):
        return AuthenticationUserSchema(email=self.request.email, password=self.request.password)


# Используем фикстуру authentication_client, которая создает нужный API клиент
@pytest.fixture
...
# Используем фикстуру private_users_client, которая создает нужный API клиент
@pytest.fixture
